...
| Model monitoring | Every backtest run logged (always on) — directional accuracy, RMSE, and drift status charted over time per ticker/model, scoped to your own account; MLflow logs the same runs too when installed |
