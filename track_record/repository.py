...
class PredictionRecordRepository(Protocol):
    ...
    def resolve(self, ticker: str, model_name: str, target_date: str, actual_close: float) -> None: ...
...
_COLUMNS = (
    "ticker", "model_name", "made_at", "target_date",
...
class SqlitePredictionRecordRepository:
        ...
        with self._lock:
            self._conn.execute(
                ...
                CREATE TABLE IF NOT EXISTS prediction_records (
                    ...
                    actual_close         REAL,
                    resolved_at          TEXT,
                    PRIMARY KEY (ticker, model_name, target_date)
                )
                """
            )
            self._conn.commit()
    ...
    def save(self, record: PredictionRecord) -> None:
        ...
        # resolves just updates the pending prediction rather than creating
        # a duplicate row. Deliberately does not touch actual_close /
        # resolved_at — a record can only move from pending to resolved via
        # resolve(), never backward.
        with self._lock:
            self._conn.execute(
                """
                INSERT INTO prediction_records
                    (ticker, model_name, made_at, target_date, last_close,
                     predicted_close, predicted_log_return)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(ticker, model_name, target_date) DO UPDATE SET
                ...
                """,
                (
                    record.ticker, record.model_name, record.made_at, record.target_date,
                    record.last_close, record.predicted_close, record.predicted_log_return,
                ),
            )
            self._conn.commit()
    ...
    def resolve(self, ticker: str, model_name: str, target_date: str, actual_close: float) -> None:
        ...
        # by a second call (e.g. resolve_pending() racing a manual call, or
        # any future caller that doesn't happen to pre-filter to unresolved
        # records the way it does today).
        with self._lock:
            self._conn.execute(
                """
                UPDATE prediction_records
                SET actual_close = ?, resolved_at = ?
                WHERE ticker = ? AND model_name = ? AND target_date = ? AND actual_close IS NULL
                """,
                (actual_close, _utcnow_iso(), ticker, model_name, target_date),
            )
            self._conn.commit()
